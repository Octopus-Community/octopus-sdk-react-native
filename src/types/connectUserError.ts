/**
 * Reason a {@link connectUser} call was refused, carried as the `code` of the rejected error.
 *
 ...
 *
 * @see {@link ConnectUserError} – the shape of the rejected error.
 * @see {@link isConnectUserError} – narrow an unknown caught value.
 */
export type ConnectUserErrorCode =
  | 'USER_BANNED'
  | 'MISSING_TOKEN'
  | 'INVALID_TOKEN'
  | 'PROFILE_ERROR'
...
/**
 * The error {@link connectUser} rejects with when the native SDK refuses the connection.
 *
 ...
 */
export interface ConnectUserError extends Error {
  /**
   * Why the connection was refused. See {@link ConnectUserErrorCode}.
   *
   ...
   * {@link ConnectUserErrorCode} values and keep a default branch.
   */
  readonly code: ConnectUserErrorCode | (string & {});
}

/**
 * Narrows a value caught from {@link connectUser} to a {@link ConnectUserError}.
 *
 * Returns `true` for any rejection carrying a string `code`, including a code not listed in
 * {@link ConnectUserErrorCode} — an unrecognised failure is still a failure and must not be
 ...
 *
 * @param error - The value caught from a rejected `connectUser` call.
 * @returns Whether `error` carries a connection-failure code.
 *
 * @example
 * ```typescript
 * try {
 ...
 * ```
 */
export function isConnectUserError(error: unknown): error is ConnectUserError {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
