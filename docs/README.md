# Documentation

Documentation for **@octopus-community/react-native**.

## Where to start

| Document | Description |
|----------|-------------|
| [Main README](../README.md) | Getting started: installation, iOS/Android setup, initialization, SSO, theming, and example app overview |
| [API Reference](api/README.md) | Full TypeScript API: functions, interfaces, types, and enumerations |
| [Push notifications](./push-notifications.md) | Integration guide for iOS and Android push notifications |
| [Sync follow groups](./sync-follow-groups.md) | Batch follow/unfollow groups API reference |
| [Example app](../example/README.md) | How to run the example app and required environment variables |
| [Contributing](../CONTRIBUTING.md) | Development workflow, scripts, and pull request guidelines |

## Official Octopus docs

For product concepts, SSO backend setup (JWT), and native SDK behaviour, see the [Octopus Developer Guide](https://doc.octopuscommunity.com/).
